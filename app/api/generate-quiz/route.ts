...
async function extractTextFromPDF(buffer: Buffer): Promise<string> {
  // Converte Buffer para Uint8Array (compatível com Blob)
...
export async function POST(request: Request) {
  try {
    ...
    // 6. VERIFICA SE A RESPOSTA DA IA DEU CERTO
    if (!respostaIA.ok) {
      const erroTexto = await respostaIA.text();
      console.error('❌ Erro DeepSeek:', respostaIA.status, respostaIA.statusText);
      console.error('📄 Detalhe:', erroTexto);
      
      let mensagemErro = 'Erro ao gerar questões com IA';
      if (respostaIA.status === 401) {
        mensagemErro = 'Chave da API inválida. Verifique sua chave DeepSeek.';
      } else if (respostaIA.status === 402) {
        mensagemErro = 'Saldo insuficiente. Adicione crédito na DeepSeek.';
      } else if (respostaIA.status === 429) {
    ...
    // 8. VERIFICA SE TEM EXATAMENTE 80 QUESTÕES
    ...
    // 9. RETORNA O SUCESSO
    console.log('✅ Sucesso!', questoes.questoes.length, 'questões geradas');
